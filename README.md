...
Remotely has a basic API, which can be browsed at https://remotely.oragetechnologies.com (or your own server instance).  Most endpoints require authentication via an API access token, which can be created by going to Account - API Access.

When accessing the API from the browser on another website, you'll need to set up CORS in appsettings by adding the website origin URL to the TrustedCorsOrigins array.  If you're not familiar with how CORS works, I recommend reading up on it before proceeding.  For example, if I wanted to create a login form on https://remotely.oragetechnologies.com that logged into the Remotely API, I'd need to add "https://remotely.oragetechnologies.com" to the TrustedCorsOrigins.
